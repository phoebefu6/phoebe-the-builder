...
| [A p-value Is a Random Variable, and the Spread of That Variable Is the Result](data-science-cookbook/p-value-dance/) | "p = 0.049" gets written up as a finding, but nobody has ever watched the same study run twice - so this replays one true effect forty thousand times… |
...
| [ANOVA Says Something Differs, and the Five Ways to Ask Which Pair Name Different Pairs](data-science-cookbook/anova-posthoc/) | The ANOVA came back significant and now someone has to say which groups differ - and the five usual ways of finding out name different pairs on the same… |
...
| [Four Procedures Are Called the T-Test, and They Do Not Have the Same Error Rate](data-science-cookbook/t-test-variants/) | "We ran a t-test" does not name a procedure. On 50 users against 10 with unequal spread, the textbook t-test calls a… |
| [Four Tests on One 2x2, and the Test You Picked Decided the Launch](data-science-cookbook/proportion-test/) | 0 of 20 converted in control, 4 of 20 in the variant: two standard tests say it moved, two say it did not - and which one you ran decides the launch. |
| [Mann-Whitney Is Not a Robust t-test, It Is a Test of a Different Hypothesis](data-science-cookbook/nonparametric-swap/) | Two defensible tests, one dataset, both significant, opposite conclusions - because "just use Mann-Whitney" swaps the question, not just the method. |
| [No Significant Difference Is Not No Difference, and the n That Makes It One Is Computable](data-science-cookbook/equivalence-test/) | The test found no significant difference and everyone read it as "the two are the same" - but a small study says that about a difference that matters… |
| [Overlapping Error Bars Are Routinely Significant, and on Paired Data the Rule Fails the Other Way](data-science-cookbook/ci-overlap-fallacy/) | "The error bars overlap, so it is not significant" throws away up to half of the real results on the chart - and on paired data it fails the other way. |
...
| [A Lead Is a Claim About a Lag, and a Lag Has to Be Estimated](analytics-engineering-bi/leading-indicator-finder/) | Revenue lands a month after anyone could have done something about it, so somebody builds a table of candidate upstream metrics, correlates each against… |
