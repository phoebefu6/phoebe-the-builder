...
def test_engine_is_extracted_not_copied() -> None:
    lib = open("overlap.py").read()
    for fn in ("def run_means(", "def exact_coverage(", "def matching_level("):
...
def test_badges_point_at_this_build() -> None:
    ...
    assert B.PATH == "data-science-cookbook/ci-overlap-fallacy"
    first = "".join(B.cells[0]["source"])
    assert B.PATH in first and "nonparametric-swap" not in first


def test_code_cells_parse_and_use_library_seeds() -> None:
    src = ""
    for cell in B.cells:
        if cell["cell_type"] == "code":
            ast.parse("".join(cell["source"]))
            src += "".join(cell["source"])
    for base in ("MEANS_SEED_BASE", "CONTAINMENT_SEED_BASE", "PAIRED_SEED_BASE"):
        assert base + " + i" in src
...
def test_generated_notebook_is_executed_with_a_chart() -> None:
    if not os.path.exists(NB):
        pytest.skip("notebook not built yet")
